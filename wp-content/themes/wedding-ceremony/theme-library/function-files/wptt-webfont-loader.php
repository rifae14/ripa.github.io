...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $wedding_ceremony_url The remote URL.
		 */
		public function __construct( $wedding_ceremony_url = '' ) {
			$this->remote_url = $wedding_ceremony_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $wedding_ceremony_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $wedding_ceremony_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $wedding_ceremony_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $wedding_ceremony_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $wedding_ceremony_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $wedding_ceremony_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $wedding_ceremony_format = 'woff2' ) {
			$this->font_format = $wedding_ceremony_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $wedding_ceremony_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $wedding_ceremony_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $wedding_ceremony_url, '/' ) ) {
				$wedding_ceremony_parsed_url = parse_url( $this->remote_url );
				return $wedding_ceremony_parsed_url['scheme'] . '://' . $wedding_ceremony_parsed_url['hostname'] . $wedding_ceremony_url;
			}

			return $wedding_ceremony_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $wedding_ceremony_url    The URL of the remote webfont.
	 * @param string $wedding_ceremony_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $wedding_ceremony_url, $wedding_ceremony_format = 'woff2' ) {
		$wedding_ceremony_font = new WPTT_WebFont_Loader( $wedding_ceremony_url );
		$wedding_ceremony_font->set_font_format( $wedding_ceremony_format );
		return $wedding_ceremony_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $wedding_ceremony_url, $wedding_ceremony_format = 'woff2' ) {
		$wedding_ceremony_font = new WPTT_WebFont_Loader( $wedding_ceremony_url );
		$wedding_ceremony_font->set_font_format( $wedding_ceremony_format );
		return $wedding_ceremony_font->get_url();
	}
}